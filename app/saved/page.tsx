...
export default function Saved() {
  ...
  async function updateStatus(savedId: number, newStatus: string) {
    await supabase.from('saved_creators').update({ status: newStatus }).eq('id', savedId)
    setSaved(saved.map((s) => (s.id === savedId ? { ...s, status: newStatus } : s)))
  }
    ...
    <main style={{ minHeight: '100vh', background: '#fafafa', fontFamily: 'system-ui, sans-serif', padding: '48px 16px' }}>
      <div style={{ maxWidth: 640, margin: '0 auto' }}>
        ...
        {loading ? (
          ...
          <>
            <p style={{ color: '#999', fontSize: 14, marginBottom: 16 }}>{saved.length} {saved.length === 1 ? 'creator' : 'creators'}</p>
            {saved.map((s) => {
              const c = s.creators
              const statusInfo = statusOptions.find((opt) => opt.value === s.status) || statusOptions[0]
              return (
                <div key={s.id} style={{ background: '#fff', borderRadius: 14, padding: 20, marginBottom: 12, boxShadow: '0 1px 3px rgba(0,0,0,0.06)' }}>
